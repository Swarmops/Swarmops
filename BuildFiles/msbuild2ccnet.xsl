	...
	<xsl:template match="msbuild">
      ...
      <xsl:choose>
				<xsl:when test="@error_count > 0">
				    (<span style="color:#C00"><xsl:value-of select="@error_count"/> ERRORS</span>)
				</xsl:when>
				<xsl:when test="@warning_count > 0">
				    (<span style="color:#B90"><xsl:value-of select="@warning_count"/> WARNINGS</span>)
				</xsl:when>
				<xsl:otherwise>
				    (<span style="color:#0D0">CLEAN BUILD</span>)
				</xsl:otherwise>
			</xsl:choose>
		</span></td></tr>
