...
// ReSharper restore CheckNamespace
{
    public partial class UploadBankFiles : PageV5Base
    {
        ...
        public static ReportedImportResults GetReportedImportResults(string guid)
        {
            ...
            ImportExternalTransactionDataResults resultDetail = GuidCache.Get(guid + "-ResultDetails") as ImportExternalTransactionDataResults;
                        ...
                        if (resultDetail.CurrencyCode != resultDetail.InitialBalanceCurrencyCode)
                        {
                            ...
                            html += " " +
                                String.Format(
                                    ...
                                    resultDetail.BalanceMismatchCents / 100.0);
                        }
                ...
                if (fileType == BankFileType.Unknown)
                {
                    using (StreamReader reader = CreateReader(uploadedDoc, externalData.Profile.Encoding))  // Guarantees disposal of reader
                    {
                        try
                        {
                            ...
                            ImportExternalTransactionDataResults results = Swarmops.Logic.Support.AsyncProcesses.ImportExternalTransactionData(externalData, new AsyncProcesses.ImportExternalTransactionDataArgs { Account = account, CurrentUser = currentUser, Guid = guid, Organization = organization });
            ...
            foreach (string line in lines)
            {
                ...
                switch (line.Substring(0, 2))
                {
                    ...
                    case "15": // End payment group
                        ...
                        {
                            tag = "SEBGM" + tag;
                            // a flag date where we add a tag for SE Bankgiro Max format, enabling other formats in other namespaces
                        }
