...
using System.Web;
using Swarmops.Interface.Localization;
    ...
    public class PirateWebMapProvider : XmlSiteMapProvider
    {
        ...
        public override SiteMapNodeCollection GetChildNodes(SiteMapNode node)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                SiteMapNode newNode = new SiteMapNode(oldNode.Provider, oldNode.Key, oldNode.Url,
                    LocalizationManager.GetLocalString(oldNode.Description, oldNode.Title), oldNode.Description);
        ...
        private SiteMapNode LocalizeNode(SiteMapNode node)
        {
            ...
            return new SiteMapNode(node.Provider, node.Key, node.Url,
                LocalizationManager.GetLocalString(node.Description, node.Title), node.Description);
        }
    }
}