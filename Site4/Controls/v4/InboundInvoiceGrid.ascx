<%@ Control Language="C#" AutoEventWireup="true" CodeFile="InboundInvoiceGrid.ascx.cs" Inherits="Controls_v4_InboundInvoiceGrid" %>
...
<MasterTableView autogeneratecolumns="False" DataKeyNames="Identity">
    <Columns>
        ...
        <telerik:GridBoundColumn HeaderText="Received" DataFormatString="{0:yyyy-MM-dd}" UniqueName="column2" DataField="CreatedDateTime" />
        <telerik:GridBoundColumn HeaderText="Payment Due" DataFormatString="{0:yyyy-MM-dd}" UniqueName="columnDue" DataField="DueDate" />
        <telerik:GridBoundColumn HeaderText="Supplier" UniqueName="column3" DataField="Supplier" />
        ...
        <telerik:GridTemplateColumn HeaderText="Paid/Closed" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" UniqueName="TemplateColumn3">
            <ItemTemplate>
                <asp:Image ID="ImageClosedPaid" runat="server" />
            </ItemTemplate>
        </telerik:GridTemplateColumn>
