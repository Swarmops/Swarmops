...
namespace Swarmops.Database
{
    public partial class SwarmDb
    {
        ...
        private const string pwEventFieldSequence =
            " PWEvents.EventId, PWEventTypes.Name AS EventType, PWEventSources.Name AS EventSource, PWEvents.ActingPersonId, PWEvents.AffectedPersonId, " +
            // 0-4
            " PWEvents.CreatedDateTime, PWEvents.ProcessedDateTime, PWEvents.Open, PWEvents.OrganizationId, PWEvents.GeographyId, " +
            // 5-9
        ...
        private BasicPWEvent ReadPWEventFromDataReader (DbDataReader reader)
        {
            ...
            return new BasicPWEvent (eventId, eventDateTime, open, processedDateTime, eventType, eventSource,
                actingPersonId,
        ...
        public BasicPWEvent[] GetTopUnprocessedEvents()
        {
            ...
            using (DbConnection connection = GetMySqlDbConnection())
            {
                ...
                DbCommand command =
                    GetDbCommand (
                        "SELECT " + pwEventFieldSequence + " WHERE Open=1 AND ProcessDateTime < NOW() LIMIT 10",
                        connection);
        ...
        public BasicPWEvent[] GetEventsForPerson (int personId)
        {
            ...
            using (DbConnection connection = GetMySqlDbConnection())
            {
                ...
                DbCommand command =
                    GetDbCommand (
                        "SELECT " + pwEventFieldSequence + " WHERE AffectedPersonId=" + personId +
        ...
        public BasicPWEvent[] GetEventsOfType (EventType eventType)
        {
            ...
            using (DbConnection connection = GetMySqlDbConnection())
            {
                ...
                DbCommand command =
                    GetDbCommand (
                        "SELECT " + pwEventFieldSequence + " WHERE PWEventTypes.Name='" + eventType +
